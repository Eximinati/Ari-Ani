...
module.exports = {
    ...
    start: async (client, m, {
        ...
        let textr = "";
        if (pushName) {
            textr += `*${pushName}#${tag}'s* Exp\n\n`
        } else {
            textr += `*${m.sender}#${tag}'s* Exp\n\n`
        }
        textr += `*🎯️XP*: ${senderUser.xp} / ${Levels.xpFor(senderUser.level + 1)}\n*❤️Level*: ${senderUser.level}\n*🔮️Role*: ${role}`
        ...
        .setAvatar(ppuser)
        .setLevel(senderUser.level)
        .setLevelColor(randomHexs, randomHex)
        .setCurrentXP(senderUser.xp)
        .setOverlay(randomHex, 100, false)
        .setRequiredXP(Levels.xpFor(senderUser.level + 1))
