...
exports.restart = () => {
  ...
  exec("pm2 restart ecosystem.config.js", (error, stdout, stderr) => {
    if (error) {
      ...
      return m.reply("An error occurred while restarting the application.");
    }
    if (stderr) {
    ...
    m.reply("The application has been restarted.");
  });
},
...
await sleep(4000)
  ...
  Promise.all([unlink(`bin/${filename}.mp4`), unlink(`bin/${filename}.gif`)])
  return buffer5
                     }

                    exports.randomRange= async (min, max) => {
